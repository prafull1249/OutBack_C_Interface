    ...
    switch (function) {
		case _FC_READ_COILS: {
			...
			if ((address + nb) > mb_mapping->nb_bits) {
				...
				rsp_length = response_io_status(address, nb,mb_mapping->tab_bits,rsp, rsp_length);
			}
		}
			break;
		case _FC_READ_DISCRETE_INPUTS: {
			...
			if ((address + nb) > mb_mapping->nb_input_bits) {
				...
				rsp_length = ctx->backend->build_response_basis(&sft, rsp);
				rsp[rsp_length++] = (nb / 8) + ((nb % 8) ? 1 : 0);
				rsp_length = response_io_status(address, nb,mb_mapping->tab_input_bits,rsp, rsp_length);
			}
		}
			break;
		case _FC_READ_HOLDING_REGISTERS: {
		...
		default:
			rsp_length = response_exception(ctx, &sft,MODBUS_EXCEPTION_ILLEGAL_FUNCTION,rsp);
			break;
    }
...
/* Reads the boolean status of bits and sets the array fields
